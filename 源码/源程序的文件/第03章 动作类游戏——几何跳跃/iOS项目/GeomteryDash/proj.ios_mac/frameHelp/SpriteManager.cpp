...
SpriteManager::SpriteManager(Layer* layerIn)
{
	...
	batchNode = SpriteBatchNode::create("GJ_GameSheet1.png",800);
...
void SpriteManager::backgroundRoll()//背景滚动方法
{
	...
	//===背景===
	Sprite* bg = createStaticSprite(new std::string("bg"), "game_bg_01_001-hd.png", 0.0f, 0.0f, 0, 0, WIDTH, HEIGHT);//创建背景精灵
	if(layer->getTag() == INDEX_GRAMLAYER)
	{
	...
	Sprite* bg1 = createStaticSprite(new std::string("bg1"), "game_bg_01_001-hd.png", 0.0f, 0.0f, WIDTH, 0, WIDTH, HEIGHT);//创建背景精灵
	if(layer->getTag() == INDEX_GRAMLAYER)
	{
