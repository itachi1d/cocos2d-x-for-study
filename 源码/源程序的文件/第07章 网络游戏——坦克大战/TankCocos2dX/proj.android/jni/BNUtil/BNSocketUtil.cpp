...
#include "mainTank\Constant.h"
#include "mainTank\SelectLayer.h"
#include "mainTank\AppMacros.h"
...
void* BNSocketUtil::threadReceiveTask()
{
	while(connectFlag)
	{
		...
		else if(strcmp(data,"<#GAME_MISSILE#>")==0)//主坦克导弹
		{
			...
			if(missileFlag==1)
			{
				Constant::playEffect((SOUND_PATH+std::string("rocket_shoot2.mp3")).c_str());
			}
			int length=receiveInt();
			float *temp=new float[length];
			for(int i=0;i<length;i++)
		...
		else if(strcmp(data,"<#GAME_EXPLOSION#>")==0)
		{
			...
			for(int i=0;i<length;i++)
			{
				...
				if(temp[i]==1)
				{
					Constant::playEffect((SOUND_PATH+std::string("grenada.mp3")).c_str());
				}
			}
			if(explosionFlag==0)
			{
