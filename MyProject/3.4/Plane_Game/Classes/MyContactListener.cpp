...
void MyContactListener::BeginContact(b2Contact* contact)
{
	...
	if((a == 1 && b == 3) | (b == 1 && a == 3))//敌机子弹碰到主角
		playerContactDiji(contact);
	if((a == 1 && b == 4) | (b == 1 && a == 4))//BOSS碰到主角
		playerContactDiji(contact);
	if((a == -2 && b == 4) | (b == -2 && a == 4))//主角子弹碰到BOSS
		atackBoss(contact);
}
...
void MyContactListener::atackBoss(b2Contact* contact)
{
	b2Body *bulletBody;
	b2Body *bossBody;
	if(a == -2)
	{
		bulletBody = contact->GetFixtureA()->GetBody();
		bossBody = contact->GetFixtureB()->GetBody();
	}
	else
	{
		bulletBody = contact->GetFixtureB()->GetBody();
		bossBody = contact->GetFixtureA()->GetBody();
	}
	b2WorldManifold *fold = new b2WorldManifold;
	contact->GetWorldManifold(fold);
	b2Vec2 contactPoint1 =  fold->points[0];
	...
	auto *scene = Director::getInstance()->getRunningScene();
	GameScene *layer = (GameScene*)scene->getChildByTag(0);
	if(layer->bossUpdate->xuetiao)
	{
		((Boss*)bossBody->GetUserData())->subLife(bullet->ATK);
		layer->bossUpdate->updateXueTiao();
	}
	
	//((Boss*)bossBody->GetUserData())->beAttackAction();
	if(bullet->TYPE == 0)
	{
		layer->addDeleteBullet(bulletBody);
		layer->showBinGoParticle(point);
